...
<div class="card-dashboard mx-auto p-4 p-md-5" style="max-width: 800px;">
    ...
    <div class="mb-4 pb-3 border-bottom text-center text-md-start">
        <h4 class="mb-1 fw-bold" style="color: #0f172a; letter-spacing: -0.5px;">Edit User</h4>
        <p class="text-muted small mb-0">Perbarui informasi akun <span class="fw-bold text-dark">{{ $user->name }}</span>.</p>
    </div>
    ...
    <form action="{{ route('users.update', $user->id) }}" method="POST">
        ...
        <div class="row g-4 mb-4">
            ...
            <div class="col-md-6">
                ...
                <input type="text" name="name" class="form-control input-ui @error('name') is-invalid @enderror" placeholder="Misal: Budi Santoso" value="{{ old('name', $user->name) }}">
                @error('name')
            ...
            <div class="col-md-6">
                ...
                <input type="email" name="email" class="form-control input-ui @error('email') is-invalid @enderror" placeholder="Misal: budi@perusahaan.com" value="{{ old('email', $user->email) }}">
                @error('email')
                    <div class="invalid-feedback fw-semibold small">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="col-md-6">
                ...
                <select name="role" class="form-select input-ui @error('role') is-invalid @enderror" style="cursor: pointer;">
                    <option value="staff" {{ old('role', $user->role) == 'staff' ? 'selected' : '' }}>Staff (edit & lihat data saja)</option>
            ...
            <!-- Kolom Kanan: Password Baru (opsional) -->
            <div class="col-md-6">
                <label class="form-label fw-bold">Password Baru <span class="text-muted fw-normal" style="text-transform: none !important;">(Kosongkan jika tidak diganti)</span></label>
            ...
            <div class="col-12">
                ...
                <input type="password" name="password_confirmation" class="form-control input-ui" placeholder="Ulangi password baru (opsional)">
            </div>
