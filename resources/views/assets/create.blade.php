...
<div class="card-dashboard mx-auto p-4 p-md-5" style="max-width: 800px;">
    ...
    <div class="mb-4 pb-3 border-bottom text-center text-md-start">
        <h4 class="mb-1 fw-bold" style="color: #0f172a; letter-spacing: -0.5px;">Tambah Aset IT Baru</h4>
        <p class="text-muted small mb-0">Lengkapi form di bawah ini untuk mendaftarkan aset ke dalam sistem.</p>
    </div>
    ...
    <form action="{{ route('assets.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="row g-4 mb-4">
            ...
            <div class="col-md-6">
                ...
                <input type="text" name="asset_code" class="form-control input-ui @error('asset_code') is-invalid @enderror" placeholder="Misal: LPT-001" value="{{ old('asset_code') }}" style="font-family: monospace; font-weight: 600; color: #4f46e5;">
                @error('asset_code')
            ...
            <div class="col-md-6">
                ...
                <input type="text" name="name" class="form-control input-ui @error('name') is-invalid @enderror" placeholder="Misal: Lenovo ThinkPad T490" value="{{ old('name') }}">
                @error('name')
                    <div class="invalid-feedback fw-semibold small">{{ $message }}</div>
                @enderror
            </div>
            ...
            <!-- Upload Foto Fisik -->
            <div class="col-md-6">
                <label class="form-label fw-bold">Foto Fisik Barang <span class="text-muted fw-normal text-capitalize" style="text-transform: none !important;">(Opsional)</span></label>
                <input type="file" name="image" class="form-control input-ui p-2 @error('image') is-invalid @enderror" accept="image/png, image/jpeg, image/jpg">
                @error('image')
                    <div class="invalid-feedback fw-semibold small">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="col-12">
                ...
                <!-- Menghapus $asset->problem_description karena belum ada data (halaman create) -->
                <textarea name="problem_description" class="form-control input-ui @error('problem_description') is-invalid @enderror" rows="3" placeholder="Misal: Layar bergaris, baterai drop, dll...">{{ old('problem_description') }}</textarea>
